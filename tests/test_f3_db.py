        ...
        warnings=[
            {
                ...
                "label": "pide autorización US",
                "evidence": "authorized to work in the US",
            }
        ],
        breakdown={
            "base": 50.0,
            "final": 71.0,
            "factors": [{"factor": "role", "delta": 25.0, "note": "role matches title"}],
        },
    )
    row = db.get_job(jid)
...
def test_add_followup_with_kind(db: DB):
    jid = _seed(db)
    fid = db.add_followup(
        jid, channel="email", touch_number=1, due_at="2026-07-11T00:00:00+00:00", kind="applied"
    )
...
def test_stories_crud_roundtrip(db: DB):
    sid = db.add_story(
        ...
        situation="ETL crítico caído",
        task="Restaurar en <1h",
        action="Rollback + circuit breaker",
        result="Recuperado en 40min",
        reflection="Alertas proactivas desde entonces",
        skills=["python", "airflow"],
    )
    s = db.get_story(sid)
    assert s["title"].startswith("Pipeline") and s["skills"] == ["python", "airflow"]
    assert (
        db.update_story(sid, {"result": "Recuperado en 35min", "skills": ["python", "sql"]}) is True
    )
    s2 = db.get_story(sid)
