...
def test_monthly_salary_annualized_meets_floor():
    from engine.scoring.fit import score_job
    job = {"title": "Senior Data Scientist", "is_remote": 1, "workplace_type": "remote",
           "description": "python and sql", "salary_max": 10000, "salary_interval": "monthly"}
...
def test_list_jobs_accepts_generator(db: DB):
    ...
    rows = db.list_jobs(states=(s for s in ["shortlisted", "ready"]))  # generator, consumed once internally
...
def test_discover_only_indeed_filters_jobspy_sites(db: DB, monkeypatch):
    ...
    runner.discover(db, sources_cfg={"jobspy": {"enabled": True, "sites": ["indeed", "linkedin"]}},
                    companies=[], terms=["data scientist"], only={"indeed"})
...
def test_pdf_renders_project_description(tmp_path: Path):
    ...
    desc = {"basics": {"name": "A"}, "projects": [{"name": "P1", "highlights": ["did x"],
